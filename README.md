...
This project is for running virtualised Linux guests on seL4 for ARM platforms. The camkes-arm-vm implements
a virtual machine monitor (VMM) server, faciliating the intialisation, booting and run-time management of a guest OS.

Currently the supported ARM platforms the camkes-arm-vm can run on include:
...
#### How do I update camkes-arm-vm to support platform 'X'
